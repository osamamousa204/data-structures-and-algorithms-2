...
const mapCurrentEvents = () => {
    let news = currentEvents.news.map(val => {
...
const countNumberOfElements = (arr) => {
    let countOverArray = arr.reduce((acc, val, idx) => {
        return acc = acc + 1
    }, 0)
    return countOverArray
};
...
const returnNames = (arr) => {
    let starwarNames = arr.reduce((acc, val, idx) => {
        acc[idx] = val.name
        return acc
    }, [])
    return starwarNames
...
const reversedString = (str) => {
    ...
    let myRevarseName = arrayName.reduce((acc,val,idx)=>{
      acc = val + acc
      return acc
    },'');
    return myRevarseName
};

...
const toLastNames = people => {
    let toLastName = people.map(val=>{
        let str = `${val.firstName} ${val.lastName}`;
        return str
      })
      return toLastName
};
...
const validateEmail = (email) => {
    let regEx = /^\b\w+.\b\w+@\w+.net\b|^\b\w+.\b\w+@\w+.org\b|^\b(\w+.\w+)@\w+.com\b|^\b\w+@\w+.net\b|^\b\w+@\w+.org\b|^\b\w+@\w+.com\b/g;
...
let regEx = /^\(\b\d{3}\b\)\s\d{3}\-\d{4}\b$|^\(\b\d{3}\b\)\d{3}\s\d{4}\b$|^\b\d{3}\b\s\d{3}\-\d{4}\b$|^\b\d{3}\b\-\d{7}\b$|^\b\d{3}\b\-\d{3}\s\d{4}\b$|^\b\d{3}\b\-\d{3}\-\d{4}\b$|^\b\d{3}\b\s\d{3}\s\d{4}\b$|^\b\d{6}\-\b\d{4}\b$|^\b\d{10}\b$|^\b\d{3}\b\s\d{7}\b$/g;

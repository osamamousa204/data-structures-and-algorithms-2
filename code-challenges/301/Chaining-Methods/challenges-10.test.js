...
const createServer = () => {
    ...
    app.get('/aboutme', (request, response) => {
        let aboutMe = 'my name is Osama .. i am 24 years old .. I studied Architectural Engineering in Philadelphia university';
        response.status(200).send(aboutMe)
    })
    app.get('/favoritefoods', (request, response) => {
        let myFavoriteFoods = ['Mansaf', 'Magloba', 'Maftool', 'Shawirmah'];
        response.status(200).send(myFavoriteFoods)
    })
    app.get('*', (request, response) => {
        let notFoundMsg = 'page not found'
...
const count = (target, input) => {
    let total = 0;
    input.forEach(val => {
        let amountOfTimes = val.reduce((acc1, val1, idx) => {
            if (val1 === target) {
                acc1 = acc1 + 1
                return acc1
            } else {

...
const toTitleCase = (arr) => {
    let arrayOfUpperCase = arr.map(val => {
        let str = val.replace(val.charAt(0), val.charAt(0).toUpperCase())
        return str
    })
...
let biggerThanLuke = (arr) => {
    let arrayOfNamesBiggerThanLuke = [];
    arr.forEach((val, idx, arr) => {
        if (parseInt(val.mass) > parseInt(arr[0].mass)) {
            arrayOfNamesBiggerThanLuke.push(val.name)
        }
    })
    let result = arrayOfNamesBiggerThanLuke.join(' - ');
    return result
};

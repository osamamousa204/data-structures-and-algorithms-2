...
const findHappiness = (arr) => {
    let happayString = arr.filter(val => {
        return val.includes(':)')
    })
    return happayString
};
...
const standardizePhoneNumbers = (arr) => {
    let arrOfPhoneNumbers = arr.map(val => {
        let splittedNum = val.split('');
        let trueNumber = splittedNum.reduce((acc, val2, idx) => {
            if (!isNaN(val2) && val2 != ' ') {
                acc = acc + val2
                return acc
            } else {
